...
## What is the value provided by the Frontier Model companies?

Arguably, the largest value proposition of any of the Frontier Model companies is the obvious one: the LLM. 

The proprietary training, weights, and the model inference infrastructure itself.

There are a LOT of other value propositions that Anthropic, Grok, Open AI, and Gemini provide - but all of those other value propositions rely on their LLM.

The *other* value propositions are becoming more and more important for these companies - we see it most with Anthropic. 

Anthropic is building a walled garden of vendor lock-in, creating the entire infrastructure for their agents to operate within. 

But, I digress. That is a post for another time. 

## Where is the value of an LLM?

Ah. The existential stuff already? Ok. 

The largest market right now for LLM usage fees are software developers. Hands down, by far, those of us burning tokens every day (and thus paying the companies for that privilege) are software developers.

The other heavy users that are starting to trend upwards are analysts, researchers, and people like creators who run multiple loops or 24/7 research bots (see [have fun good night]()) to get to their conclusions.

Ignoring my obvious heavy bias, we'll focus on developers.

## An LLM takes in strings, and returns strings.

As a developer, is the LLM the value? Not at all. I can't code with an LLM. An LLM can only receive and emit strings. Not useful.

The harness itself is the value for me - it is the arms and legs of the LLM. It can do the things I want done. It turns strings that represent a tool the LLM wants to call, and calls the tool. That's the job of the harness - turn strings into actions.
...
The only real moat right now is the developer opinion on the quality of the code that an LLM produces, and vendor lock-in. 

But this post isn't about moats or vendor lock-in. This post is about Local LLMs. 

## No moat. So why doesn't everyone build a Frontier Model?
...
Ok, that's reductionist. Here's a good quote from [Together.ai]() about the absurdity of locking all this down in proprietary landscapes:
...
So eventually, all these developers are going to wake up and realize they can control their LLM deployment just like they can control their API routes.

And every developer who has had to deploy an agent into the wild knows the feeling of having your agent perform well one day, and then the next it doesn't, only to realize the model has obviously changed. 
...
> Side note: most businesses also want to operate on their data using an LLM safely. Private, controlled deployments in hyperscaler clouds are becoming the norm, and setting up any model (FM or open source) is a few clicks.
...
Apple and Google will eventually and already are going to bake an LLM into the OS itself, running on the device (our phones today are incredibly capable devices and have good GPUs to make use of).
...
But that is my general point writ large, it is my main thesis for this blog: the LLM isn't as revolutionary as people are blowing it up to be. 

The LLM will quickly become just another tool in the toolbox for developers and creators to use, as boring as implementing a REST API.

## LLMs are the new REST APIs

For LLMs to do anything useful, code and developers are required to create something that can solve the problem or achieve the goal - consistently and reliably.

LLMs are quickly shifting from this magical land of opportunity, to a tool we need to control as a developer.

Our bosses aren't so accepting of 80% accuracy, or context limits, or any of the many things we need to deal with when building agents. 
...
LLMs will more and more often get tied into aiding or completely automating critical processes. 
...
And as open source models and finetuning techniques get better and better, eventually these open-source LLMs and controlled deployments will become the defacto choice. 
...
To move into another leap, we're going to need more than just LLM tech as it is today. Fundamental tech shifts like what the transformers and pytorch libraries did for LLMs, something that bakes how the world works into these algorithms (world models).
...
If you don't want to run things locally but still try an open source model, a good and affordable option I've used in the past is a [Chutes.ai]() or using an [OpenRouter]() subscription. 
...
FM companies or hyperscalers or enterprises or even solo developers cannot proceed without those GPUs. 
...
Once local LLMs get small enough and capable enough to run on the edge, device makers enabling this also benefit from all this pressure, but this will happen much later (3-5 years out imo - the average time where anything is possible in tech :D).
