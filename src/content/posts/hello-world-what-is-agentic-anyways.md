...
The basic interpretation is that an AI agent is going to be able to do anything for anyone and magically fix all our business problems - and the demand for AI because of those agentic tools will explode. 

But... what IS it?

## Fancy words for simple ideas

In my humble opinion, an agent is any process that receives input, produces output, and uses an AI/LLM in some way inside that process. 

That sounds... like every single AI deployment you've ever heard of, right? You'd be correct, again in my opinion. 

So why the hype machine exploding at the seams to get everyone using and making agents?

## A (very) brief history. 

This is the natural evolution for every programmer who has ever had to build an LLM chat and bake that into their application. 
...
So if we REALLY want to demystify this, an Agent is a program (code) that integrates an LLM in such a way that the LLM can execute code, "run" tasks, etc. 

LLMs cannot ACTUALLY do this - they still only produce text (or images, or video, or other 'modalities'). So how do they use these "tools"?
...
For this to work, you need an LLM that can read and write these function calls and choose when to do so, and you need a "harness" - a term becoming popular for referring to the code that surrounds all these AI calls. 

## So, what's a harness? Is a harness an agent?

A harness is any code or process that wraps LLM calls, typically in a loop, and provides things like tools, instructions, etc. 

Sounds pretty much like an agent? You'd be right to assume that, in my view of the world. 

Harness Engineering is the new fancy words for everything we're talking about now, and in my view, an Agent is just another kind of harness. 

Bringing all this together, there are a few very popular ways these Agents/Harnesses are being used. 

## Agentic Development

In general, developers who are wholesale adopting AI in their workflows are using tools like [OpenCode](), [Claude Code](), [Pi](), and other developer-focused harnesses. 

These harnesses are purpose-built to help a developer guide AI to write code, run it, test it, and deploy it. Developers taking advantage of these types of tools are generally able to produce MORE code, with many developers dubious of the quality (not this developer - there are ways to mitigate problems now today). 

## Purpose Built Agents

Let's say you wanted your chat bot to DO something for you - take an order, let's say. 

A purpose-built agent would be designed to use a framework like [langchain](), [agent-core](), or my personal favorite, [dspy](). These frameworks provide enough code for the developer to quickly write a workflow - more akin to basic instructions to follow: when given X input, follow Y instructions, and produce Z output. 
...
For instance, now in [Amazon Connect](), an AWS SaaS that allows you to build the infrastructure powering your call center easily, you can allow AI to do what's called "slot filling", or what allows you to receive someone's birth date, reason for calling, first name, and last name, in a FAR better experience than dealing with the technology of yore.

Other things I've deployed are tools that allow an AI to review a call's transcript, follow a complex set of rules absolutely custom to the business, and score the quality of the call.  

## But then, why is everyone saying we will have 24/7 assistants?

In other words, you have heard about [Open Claw]()? 

If not, it exploded onto the internet in 2025. It allows you to have an LLM periodically be "woken up", read a "SOUL.md" markdown file, and follow the instructions. Add function calling/tools into the mix, and you have a fully autonomous agent periodically doing things. But the headlines can show you the hypemachine in action:
...
This post is long enough already, so I'll wind this down, but I could go on and on why 24/7 "lobster" AIs is a terrifically bad idea. I'll save that for another post though. 

But if you've made it this far, my hot take on all this is: Purpose-built agents, which can be tested and evaluated, optimized, and reliably produce proper workflow outputs is what business _actually_ wants. The reliability is really what they are after, and that is extremely difficult when you try to contain a "lobster". 

No one wants to be woken up to a nightmare for how your agent just "decided" to delete customer data. No one wants to wake up and see that the agent made irreversible actions happen. Most business leaders don't even want to see a single error from the LLM in our post-deterministic world, but that too is another post for another day. 
...
If all three of those were yes, then you need something more containable then letting loose a harness connected to every API you can control, and you can probably just have a coding harness develop a script for you to run on a periodic cron job. 

For almost any other combination, it's easier to just have a code harness whip out a one-off workflow for you. 

When you answer yes to those three above, then your best bet is to pick one of the excellent agentic frameworks out there, and get to prompt engineering, eval'ing, and optimizing to get that accuracy score up. 
